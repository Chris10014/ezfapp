...
const addParticipant = async (participant) => {
  const resp = await axios.post(serverUrl + "participants", participant)  
  return resp
}
...
const RegisterForm = (props) => {  
  const formik = useFormik({
    initialValues: {
      race: props.eventDateToRegister?.races[0]._id, // [index] must be [0] in case only one race exists in the object array
      firstName: "",
      lastName: "",
      hideLastName: false,
      email: "",
    ...
    validationSchema: Yup.object({
      race: Yup.string().required("Bitte Wettkampf auswählen"),
    ...
    onSubmit: (values) => {
      if (values.team && values.team.length > 0) {
        const teamExists = props.teams?.data.filter((team) => team.name.toLowerCase() === values.team.toLowerCase())[0];
        if (teamExists) {
          console.log("onSubmit replaced teamName with teamId: ", teamExists);
        } else {
          //handle unknown teams
          console.log("Team " + values.team + " ist nicht bekannt.");
          //   values.team = null;
        }
      }
      console.log("send to createParticipant: ", JSON.stringify(values));
  ...
  const { isLoading, isError, error,  mutate } = useMutation(addParticipant, {
    onSuccess: (data) => {
    ...
    onError: (error) => {
      ...
      console.log("resp: ", error.response.data.message);
